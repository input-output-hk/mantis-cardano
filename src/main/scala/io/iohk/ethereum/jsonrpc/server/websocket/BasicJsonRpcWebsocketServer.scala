...
abstract class BasicJsonRpcWebsocketServer(
  ...
  private val pubSubActor = system.actorOf(PubSubActor.props(appStateStorage, blockchain))
...
object JsonRpcWebsocketServer {
  ...
  def apply(jsonRpcController: JsonRpcController,
    ...
    config.mode match {
      case "http" => Right(new HttpJsonRpcWebsocketServer(jsonRpcController, blockchain, appStateStorage, config))
      case "https" => Right(new HttpsJsonRpcWebsocketServer(jsonRpcController, blockchain, appStateStorage, config, secureRandom))
